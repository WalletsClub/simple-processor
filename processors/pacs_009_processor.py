...
class Pacs009Processor(object):

    def __init__(self):
        pass

    @intercept(catch=Exception, handler=on_exception)
    def process_pacs_009_from_net(self, pacs_009_message):
        ...
        if response.status_code == 200:
            print(s('Ack pacs.002 for pacs.009 succeeded'))
        else:
            print(f('Ack pacs.002 for pacs.009 failed'))
